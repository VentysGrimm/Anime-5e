...
When a standalone module is created, move its source data out of `data/sources/supplements/` into that module's own source manifest and keep the same source IDs where possible. Standalone module packages live under `modules/`.

...
- Folstavia now has a standalone setting module under `modules/`, with source-tagged Species, Classes, Attributes, Techniques, vehicles, weapons, monsters, named NPCs, faction/region journals, setting overview journals, and a local-only Folstavia Map Pack reference journal.
...
- Regression fixtures now validate a 1st-level scratch character, Game Screen Adventure pregenerated character stats and attacks, core monster attack data, Table 33 Allies and Enemies NPC actors, core neomorph variants, Mounts and Monsters actor summaries, Monstrum Libri creature attacks, the issue #35 supplemental content checklist, standalone module source separation, and standalone module source manifests through `tools/validate-regression-fixtures.mjs`.
...
- Attribute and Weapon Enhancement/Limiter assignments now have a shared mechanic layer: references preserve source rules notes, item sheets show mechanic rows, effective Rank drives derived Attribute effects and Weapon Attribute damage, Area/Range/Targets/Duration feed scope and tracking automation, and Deplete Limiters require Energy tracking.
- Actor sheets expose Anime 5E roll dialogs from Quick Rolls, using the same ability/proficiency/DC/contest path as the sheet controls.
- Items of Power now expose contained Attribute/Defect bookkeeping fields and a calculated construction point summary.
- Monster, NPC, and companion basic actor sheets now expose source-backed challenge, XP, movement, combat, and point summaries; they can roll ability checks, quick checks, contests, saving throws, skill items, common attacks, and damage. The Encounter Threat settings dialog derives Challenge Rating and XP from opponent Points, compares party and encounter threat, and still accepts manual threat and XP budget overrides.
...
- Actor sheets now include Core Rules combat manoeuvre controls. They can post source-backed manoeuvre reminders, roll a selected common attack with manoeuvre advantage/disadvantage guidance, and track target-facing tactical or grappling state for GM adjudication.
- Tracked Aim and Wait for Opening states apply prepared tactical advantage to subsequent attack rolls until cleared. Tracked Grapple, Lock, Throw, Pin, and Disarm by Grapple states keep the current target and source-backed effect notes visible on the actor sheet.

...
- Attribute and Weapon point customization now supports effective Rank, Enhancement/Limiter references, final-cost overrides, and override-note warnings. Core Attribute Enhancements follow Chapter 7 Table 18, and Enhancements/Limiters recalculate effective Rank while point cost stays tied to actual Rank unless a manual override is present. Derived Attribute automation now uses effective Rank for active effects, Weapon Attribute damage, Dynamic Powers effect limits, companion/minion scale, and item-use summaries.
